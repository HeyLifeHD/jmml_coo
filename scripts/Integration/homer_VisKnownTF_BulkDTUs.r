...
#load rna seq data
DTU_results_list<- readRDS(file.path(PostDE.dir, "StageR_adj_results.rds"))
DTU_results_list<- list("EpigenotypeHM"=DTU_results_list)
...
knownRes <- knownRes[grep("customBG",knownRes, invert=TRUE)]
...
#plot tf motifs with tf expression being significant
DAR_list_sub <- list()
DAR_list_sub_sig <- list()
DAR_list_sub_plot<-list()
#sub necessary comparisons
DAR_list_sel <- DAR_list[names(DTU_results_list)]
#plot results
for (i in names(DAR_list_sel)){
    ...
    result.df<- DTU_results_list[[i]]
    result.df <- result.df[which(result.df$gene < 0.1),]
    result.df$gene_id  <- toupper(result.df$gene_id)
    x$known_motif_table <- x$known_motif_table[which(x$known_motif_table$MotifName %in% unique(result.df$gene_id)), ]
    ...
    #x <- x[which(x$q_value_benjamini < 0.05),]
    x
    })
    ...
    pdf(file.path(analysis.dir,"homer", paste0("HomerBubble_all_qval0.05_DTUPadj0.1.pdf")), height=14)
...
#plot tf expression of significant TFs
adjp <- 0.05
known_TF_sig<- list()
...
for(i in names(DAR_list_sel)){
    ...
    for(j in names(DAR_list_sel[[i]])){
        ...
        known_TF$MotifName <-  toupper(sapply(strsplit(known_TF$MotifName ,"/", fixed=TRUE),`[`, 1))
...
col <- col[c(1,3)]
for(i in names(DTU_results_list)){
    TF <- c(known_TF_sig_all_mgi_symbol[[i]]$hypo,known_TF_sig_all_mgi_symbol[[i]]$hyper)
    plot <- DTU_results_list[[i]][toupper(rownames(DTU_results_list[[i]]))%in%TF, ]
    plot$SYMBOL <- rownames(plot)
    plot$Change <- ifelse(plot$log2FoldChange>0, "blue","red")

    pdf(file.path(analysis.dir,"homer", paste0("DTU_TF_expr_qval",adjp,"_TF_barplot.pdf")), width=4)
