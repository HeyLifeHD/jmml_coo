...
comp_oi <- c( "JMML", "FCA", "HCA_CB", "JUV", "HCA_BM")
...
for(i in unique(pvalues_melt_comb$sender)){
    ...
    plot$id_cp_interaction <- ordered(plot[order(plot$id_cp_interaction, match(plot$comparison, c( "JMML", "FCA", "HCA_CB", "JUV", "HCA_BM"))),]$id_cp_interaction)
    plot$logpval <- -log10(plot$pvalue)
    plot$log2means <- log2(plot$V2)
    length(unique(plot$interacting_celltype))

    #plot
    dir.create(file.path(output.dir, "JMML_Norma_complete_comparison", i), recursive=TRUE)
    pdf(file.path(output.dir, "JMML_Norma_complete_comparison" ,i, "test.pdf"), height=ifelse(length(unique(plot$interacting_pair))/1< 22,22,length(unique(plot$interacting_pair))/1) , width=ifelse(length(unique(plot$interacting_pair))/2< 10,10,length(unique(plot$interacting_pair))/2) )
