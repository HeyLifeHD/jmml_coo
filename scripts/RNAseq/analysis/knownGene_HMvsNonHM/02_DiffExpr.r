...
DEG_results_list_plot <- lapply(DEG_results_list_plot, function(x){
  x <- as.data.frame(x)
  x$col <- "black"
  x$col <- ifelse(test = x$padj < 0.05 & x$log2FoldChange>lfc, yes = "red", 
                  no = ifelse(test = x$padj < 0.05  & x$log2FoldChange<(-lfc), yes = "blue", "black"))
  temp <- c(head(x[x$col=="red",]$gene_id, 5), head(x[x$col=="blue",]$gene_id, 5))
  #temp <-head(x[x$gene_name %in% CTA[,1],]$gene_name,5)
  x$SYMBOL <- NA
  x[x$gene_id %in% temp,]$SYMBOL <- x[x$gene_id %in% temp,]$gene_id
...
) + rremove("legend") + geom_vline(xintercept=c(-lfc,lfc), linetype = 2) +geom_hline(yintercept=1.3, linetype = 2)
...
#heatmap
mat_genes<- assay(vst)
col <- c(HM = "#c33126", IM = "#fbbb25", LM = "#0058b4")
...
nrow(DEG_results_list$HM_vs_non_HM[which(DEG_results_list$HM_vs_non_HM$padj < 0.05 & DEG_results_list$HM_vs_non_HM$log2FoldChange>5),])

paste0(DEG_results_list$HM_vs_non_HM[which(DEG_results_list$HM_vs_non_HM$padj < 0.05 & DEG_results_list$HM_vs_non_HM$log2FoldChange>0),]$gene_id, collapse=", ")
paste0(DEG_results_list$HM_vs_non_HM[which(DEG_results_list$HM_vs_non_HM$padj < 0.05 & DEG_results_list$HM_vs_non_HM$log2FoldChange<0),]$gene_id, collapse=", ")
...
#for(i in c(6)){
    ...
    anno_colors <- list(Epigenotype=col, cluster=color_cluster)
    #heatmap
    p <- pheatmap(plot, scale="row", show_colnames=F,color= c(hcl.colors(20,"Blues"),rev(hcl.colors(20,"Reds"))),#labels_row=annorow,
                        annotation_col=as.data.frame(annovst),
                        annotation_colors=anno_colors, show_rownames=F, clustering_distance_rows="correlation" ,
...
for(i in unique(clust_genes_df$cluster)){
  ...
  clust_expr_mean[[i]]<- data.frame(AverageExpression=  clust_expr_mean[[i]], Epigenotype=as.character(anno$Epigenotype))
...
ggboxplot(clust_expr_mean_unlist, x="Epigenotype", y="AverageExpression", 
  color="Epigenotype", facet.by="cluster", palette=col) +
  rotate_x_text(angle = 90) + stat_compare_means(label.y = .1, size=2.5)
dev.off()
#in 2 row visualization
g <- ggboxplot(clust_expr_mean_unlist, x="Epigenotype", y="AverageExpression", order=c("DMSO", "DAC", "SB939","DACandSB939"),
  color="Epigenotype", palette=col) +
  rotate_x_text(angle = 90) + stat_compare_means(label.y = .1, size=2.5)
