...
dmr_LMC_list_promoters_unlist <- readRDS(file.path(analysis.dir, "medecom","downstream_combined_K3_lambda0.01", "dmr_LMC_list_promoters_unlist.rds"))
...
test <- list(dmr_LMC_05, dmr_LMC_07, dmr_LMC_08)
names(test)<- c("dmr_LMC_05_combined","dmr_LMC_07_combined","dmr_LMC_08_combined")
#Run Enrichment
results_Core <- list()
...
results_msigdb <- list() 
for (i in names(test)){
dir.create(file.path(analysis.dir,"medecom", "downstream_combined_K3_lambda0.01", "LOLA",i), recursive=TRUE)
...
dir.create(file.path(analysis.dir,"medecom", "downstream_combined_K3_lambda0.01", "LOLA_resultTables"))
saveRDS(results_regionDB, file.path(analysis.dir, "medecom", "downstream_combined_K3_lambda0.01", "LOLA_resultTables", "results_regionDB_combined.rds"))

results_regionDB<- readRDS(file.path(analysis.dir,"medecom", "downstream_combined_K3_lambda0.01",  "LOLA_resultTables", "results_regionDB_combined.rds"))
...
for(i in names(results_regionDB)){
    ...
    dir.create(file.path(analysis.dir,"medecom", "downstream_combined_K3_lambda0.01", "LOLA",i))
    for(j in unique(unique(result$collection))){
        ...
        pdf(file.path(analysis.dir,"medecom", "downstream_combined_K3_lambda0.01", "LOLA",i, paste0("EnrichLOLA_", j, ".pdf")), width=15, height=20)
        print(g)
        dev.off()
    }
}