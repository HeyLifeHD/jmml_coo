...
dmr_LMC_list_promoters_unlist <- readRDS(file.path(analysis.dir, "medecom","downstream_combined_K8_lambda0.001", "dmr_LMC_list_promoters_unlist.rds"))
...
dmr_LMC_03<- dmr_LMC_list_unlist[grep("LMC_04", names(dmr_LMC_list_unlist))]
dmr_LMC_04<- dmr_LMC_list_unlist[grep("LMC_03", names(dmr_LMC_list_unlist))]

test <- list(dmr_LMC_05, dmr_LMC_03, dmr_LMC_04)
names(test)<- c("dmr_LMC_05_combined","dmr_LMC_03_combined","dmr_LMC_04_combined")
#Run Enrichment

results_regionDB <- list() 
for (i in names(test)){
dir.create(file.path(analysis.dir,"medecom", "downstream_combined_K8_lambda0.001", "LOLA",i), recursive=TRUE)
...
dir.create(file.path(analysis.dir,"medecom", "downstream_combined_K8_lambda0.001", "LOLA_resultTables"))
saveRDS(results_regionDB, file.path(analysis.dir, "medecom", "downstream_combined_K8_lambda0.001", "LOLA_resultTables", "results_regionDB_combined.rds"))

results_regionDB<- readRDS(file.path(analysis.dir,"medecom", "downstream_combined_K8_lambda0.001",  "LOLA_resultTables", "results_regionDB_combined.rds"))
...
for(i in names(results_regionDB)){
    ...
    dir.create(file.path(analysis.dir,"medecom", "downstream_combined_K8_lambda0.001", "LOLA",i))
    for(j in unique(unique(result$collection))){
        ...
        pdf(file.path(analysis.dir,"medecom", "downstream_combined_K8_lambda0.001", "LOLA",i, paste0("EnrichLOLA_", j, ".pdf")), width=15, height=20)
        print(g)
        dev.off()
    }
}