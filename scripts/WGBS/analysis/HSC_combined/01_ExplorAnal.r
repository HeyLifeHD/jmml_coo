...
analysis.dir <-  "/home/heyj/omics/groups/OE0219/internal/jmmlc_pbat/data/odcf_md/analysis/200830_ExplorAnal_HSC_comb"
...
merged_names <- c(sapply(strsplit(colnames(bsseq_all)[1:21], "_", fixed=TRUE),function(x)paste0(x[1], "_", x[2], "_", x[3])), colnames(bsseq_all)[22:40])
bsseq_all_merged <- collapseBSseq(bsseq_all,merged_names)

#save data1
bsseq_all <- bsseq_all_merged
#add tissue and donor information
pheno <- pData(bsseq_all)
pheno$Tissue<- c(rep("tumor",15), rep("cordblood",4 ), rep("adult_bonemarrow", 15))
pheno$Donor <- c(as.character(pheno$Patient[1:15]),rep("cordblood",4 ), rep("adult_bonemarrow", 15)) 
pheno$Epigenotype <- as.character(pheno$Epigenotype)
pheno[pheno$Patient %in% c("I217", "D217"),]$Epigenotype <- "IM"
pheno$Epigenotype  <- gsub("control_CB", "wildtype",pheno$Epigenotype )
pheno$Epigenotype  <- gsub("control_adult", "wildtype",pheno$Epigenotype )
pheno$Epigenotype <- as.factor(pheno$Epigenotype)

pData(bsseq_all)<- pheno
saveRDS(bsseq_al, file.path(input.dir ,"bsseq", "bsseq_HSC_comb_snpRemoved_repMerged.rds"))
