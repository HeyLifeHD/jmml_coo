...
files <- list.files(path="/omics/groups/OE0219/internal/jmmlc_pbat/data/expression/200825_DEG/HC/", pattern=".rds",recursive=TRUE, full.names=TRUE)
temp <- strsplit(files, "-", ,fixed=TRUE)
temp  <- sapply(temp, function(x)x[length(x)])
temp <- sapply(strsplit(temp, ".", ,fixed=TRUE), "[", 1)
cell <- sapply(strsplit(files, "/", ,fixed=TRUE), "[", 12)
    ...
    for(cell in names(expr_sep)){
        ...
        dir.create(file.path(analysis.dir, "scatter_plots", "allDMRs", i ,cell), recursive=TRUE)
            for(j in names(expr_sep[[cell]])){
                ...
                merged[[i]][[cell]][[j]]$Col <- ifelse(merged[[i]][[cell]][[j]]$p_val_adj < 0.05 & merged[[i]][[cell]][[j]]$avg_logFC >1 & merged[[i]][[cell]][[j]]$pct.1 >0.1 & merged[[i]][[cell]][[j]]$pct.2 >0.1, "sign. upregulated",ifelse(merged[[i]][[cell]][[j]]$p_val_adj < 0.05 & merged[[i]][[cell]][[j]]$avg_logFC<(-1)& merged[[i]][[cell]][[j]]$pct.1 >0.1 & merged[[i]][[cell]][[j]]$pct.2 >0.1, "sign. downregulated", "no sign. change"))
                ...
                write.table( merged[[i]][[cell]][[j]], file.path(analysis.dir, "scatter_plots", "allDMRs", i ,cell, paste0("expression_methylation_",j,".txt")), quote=FALSE, sep="\t", row.names=FALSE)
            }
    }
}
...
for(i in names(merged)){
    ...
    for(j in names(merged[[i]][[cell]])){
        ...
        pdf(file.path(analysis.dir, "scatter_plots", "allDMRs", i ,cell, paste0("expression_methylation_",j,".pdf")), width=4,height=4)
            print(g)
        dev.off()
        g <- ggscatter(merged[[i]][[cell]][[j]], x = "avg_logFC", y = j,xlab=paste0("Gene expression change\n[log2foldchange]","\n",name),
            ylab=paste0("Methyladion difference","\n",name),
            ...
            geom_vline(xintercept=0, linetype = 2) +
            geom_hline(yintercept=0, linetype = 2) + rremove("legend")
        pdf(file.path(analysis.dir, "scatter_plots", "allDMRs", i ,cell, paste0("expression_methylation_",j,"_label.pdf")), width=14,height=14)
            print(g)
        dev.off()
        print(j)
    }
...
for(i in names(merged)){
    for(cell in names(merged[[i]])){
            dir.create(file.path(analysis.dir, "scatter_plots", "PromoterDMRs", i, cell ), recursive=TRUE)
    for(j in names(merged[[i]][[cell]])){
        name <- gsub("_", " ", j)
        ...
        g <- ggscatter(merged_sub, x = "avg_logFC", y = j,xlab=paste0("Gene expression change\n[log2foldchange]","\n",name),
            ...
            geom_vline(xintercept=0, linetype = 2) +
            geom_hline(yintercept=0, linetype = 2) + rremove("legend")
        pdf(file.path(analysis.dir, "scatter_plots", "PromoterDMRs", i ,cell, paste0("expression_methylation_",j,".pdf")), width=4,height=4)
            print(g)
        dev.off()
        g <- ggscatter(merged_sub, x = "avg_logFC", y = j,xlab=paste0("Gene expression change\n[log2foldchange]","\n",name),
            ylab=paste0("Methyladion difference","\n",name),
            ...
            geom_vline(xintercept=0, linetype = 2) +
            geom_hline(yintercept=0, linetype = 2) + rremove("legend")
       # pdf(file.path(analysis.dir, "scatter_plots", "PromoterDMRs", i ,cell, paste0("expression_methylation_",j,"_label.pdf")), width=14,height=14)
       #     print(g)
       # dev.off()
        print(j)
    }
