...
dmrs_final <- readRDS(dmrs_final,file.path(analysis.dir, "dmrs_gr_sub_MethDiff_anno_clust6_annotattion.rds"))
    ...
    knownRes <- knownRes[grep("homer_CD34_Strat",knownRes, invert=TRUE)]
    #knownRes <- knownRes[grep("hierachy",knownRes, invert=FALSE)]
...
names_knownRes_sub <- sapply(names_knownRes, function(x) paste0(x[11],"_",x[13]))
...
for(i in unique(sapply(names_knownRes, function(x) x[11]))){
    DAR_list[[i]]<- homer_list[grep(paste0("^",i,"_cluster"), names(homer_list))]
    temp <- strsplit(names(DAR_list[[i]]), "_", fixed=TRUE)
    temp <- sapply(temp, function(x)paste0("cluster_",x[length(x)]))
...
for (i in names(DAR_list)){
    ...
    pdf(file.path(analysis.dir,i,"homer_cluster", paste0("HomerBubble_all_qval0.05.pdf")), height=40)
...
for (i in names(DAR_list)){
    ...
    pdf(file.path(analysis.dir,i,"homer_cluster", paste0("HomerBubble_top10_qval0.05.pdf")), height=7)
    print(bubblePlot(DAR_list_sub_plot[[i]])+ggtitle(i))
    dev.off()
