...
dmr_LMC_list_unlist <- readRDS(file.path(analysis.dir, "medecom","downstream_K3_lambda0.001", "dmr_LMC_list_unlist.rds"))
dmr_LMC_list_promoters_unlist <- readRDS(file.path(analysis.dir, "medecom","downstream_K3_lambda0.001", "dmr_LMC_list_promoters_unlist.rds"))
...
results_msigdb <- list() 
for (i in names(test)){
dir.create(file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001", "LOLA",i), recursive=TRUE)
...
dir.create(file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001", "LOLA_resultTables"))
saveRDS(results_regionDB, file.path(analysis.dir, "medecom", "downstream_K3_lambda0.001", "LOLA_resultTables", "results_regionDB_combined.rds"))

results_regionDB<- readRDS(file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001",  "LOLA_resultTables", "results_regionDB_combined.rds"))
...
for(i in names(results_regionDB)){
    ...
    dir.create(file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001", "LOLA",i))
    for(j in unique(unique(result$collection))){
        ...
        pdf(file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001", "LOLA",i, paste0("EnrichLOLA_", j, ".pdf")), width=15, height=20)
        print(g)
        dev.off()
    }
}