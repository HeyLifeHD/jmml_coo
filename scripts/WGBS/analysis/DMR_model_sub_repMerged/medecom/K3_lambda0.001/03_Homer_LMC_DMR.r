...
#create bg
dir.create(file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001", "homer"))
...
write.table(data.frame(chr=dmrs_red_anal_df$seqnames, 
        ...
        file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001", "homer", "dmrs_red_anal_df.txt"),
...
for(i in names(dmr_LMC_list_unlist_hypo)){
    ...
    dir.create(file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001", "homer",name, "hypo"), recursive=TRUE)
    #hypo
    write.table(data.frame(chr=dmrs_final_df[[i]]$seqnames, 
        ...
        file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001", "homer",name, "hypo", paste0("DMRs.bed")),sep="\t", quote=FALSE, row.names=FALSE, col.names = FALSE)
}
...
for(i in names(dmr_LMC_list_unlist_hyper)){
    ...
    name <- strsplit(i, ".", fixed=TRUE)
    name <- sapply(name, function(x)paste0(x[1], x[2]))
    dir.create(file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001", "homer",name, "hyper"), recursive=TRUE)
    #hypo
    write.table(data.frame(chr=dmrs_final_df[[i]]$seqnames, 
        start=dmrs_final_df[[i]]$start, end=dmrs_final_df[[i]]$end, 
        id=dmrs_final_df[[i]]$peak_id, notUsed=NA, strand="+"),
        file.path(analysis.dir,"medecom", "downstream_K3_lambda0.001", "homer",name, "hyper", paste0("DMRs.bed")),sep="\t", quote=FALSE, row.names=FALSE, col.names = FALSE)
}
