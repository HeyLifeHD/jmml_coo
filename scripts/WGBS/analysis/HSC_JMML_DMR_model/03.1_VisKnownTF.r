...
#add reduced datagan for common analysis
...
for (i in names(DAR_list)){
    ...
    pdf(file.path(analysis.dir,i,"homer", paste0("HomerBubble_all_qval0.05.pdf")), height=40)
...
for (i in names(DAR_list)){
    ...
    pdf(file.path(analysis.dir,i,"homer", paste0("HomerBubble_top10_qval0.05.pdf")), height=7)
    print(bubblePlot(DAR_list_sub_plot[[i]])+ggtitle(i))
    dev.off()
...
#after subsetting hierachy
#read in homer results
knownRes <- list.files(file.path(analysis.dir), pattern="knownResults$", full.names = TRUE, recursive = TRUE, include.dirs= TRUE)
knownRes <- knownRes[grep("homer_CD34_Strat",knownRes, invert=TRUE)]
knownRes <- knownRes[grep("hierachy",knownRes, invert=TRUE)]
knownRes <- gsub("knownResults", "", knownRes)
knownRes <- knownRes[grep("all/$",knownRes, invert=TRUE)]
knownRes
