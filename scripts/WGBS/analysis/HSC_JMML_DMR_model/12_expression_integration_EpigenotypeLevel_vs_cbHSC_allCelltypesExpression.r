...
files <- list.files(path="/omics/groups/OE0219/internal/JMMLC/scRNA_Data/New/DEG/JMML-HCA_CB_Sampled-Epigenotype", pattern=".rds",recursive=TRUE, full.names=TRUE)
    ...
    for(cell in names(expr_sep)){
        ...
        dir.create(file.path(analysis.dir, i, "expr_integration", "allDMRs", cell), recursive=TRUE)
            for(j in names(expr_sep[[cell]])){
                ...
                write.table( merged[[i]][[cell]][[j]], file.path(analysis.dir, i, "expr_integration", "allDMRs", cell, paste0("expression_methylation_JMML-HCA_CB_Sampled-Epigenotype",j,".txt")), quote=FALSE, sep="\t", row.names=FALSE)
            }
    }
}
...
for(i in names(merged)){
    ...
    for(j in names(merged[[i]][[cell]])){
        ...
        pdf( file.path(analysis.dir, i, "expr_integration", "allDMRs", cell, paste0("scatter_expression_methylation_JMML-HCA_CB_Sampled-Epigenotype_",j,".pdf")), width=4,height=4)
            print(g)
        dev.off()
        g <- ggscatter(merged[[i]][[cell]][[j]], x = "avg_logFC", y = j,xlab=paste0("Gene expression change\n[log2foldchange]","\n",name),
            ylab=paste0("Methyladion difference","\n",name),
            ...
            geom_vline(xintercept=0, linetype = 2) +
            geom_hline(yintercept=0, linetype = 2) + rremove("legend")
        pdf( file.path(analysis.dir, i, "expr_integration", "allDMRs", cell, paste0("scatter_expression_methylation_JMML-HCA_CB_Sampled-Epigenotype_",j,"_label.pdf")), width=4,height=4)
            print(g)
        dev.off()
        print(j)
    }
...
for(i in names(merged)){
    ...
    for(j in names(merged[[i]][[cell]])){
        ...
        g <- ggscatter(merged_sub, x = "avg_logFC", y = j,xlab=paste0("Gene expression change\n[log2foldchange]","\n",name),
            ...
            geom_vline(xintercept=0, linetype = 2) +
            geom_hline(yintercept=0, linetype = 2) + rremove("legend")
        pdf( file.path(analysis.dir, i, "expr_integration", "PromoterDMRs", cell, paste0("scatter_expression_methylation_JMML-HCA_CB_Sampled-Epigenotype_",j,".pdf")), width=4,height=4)
            print(g)
        dev.off()
        ...
        g <- ggscatter(merged_sub, x = "avg_logFC", y = j,xlab=paste0("Gene expression change\n[log2foldchange]","\n",name),
            ...
            geom_vline(xintercept=0, linetype = 2) +
            geom_hline(yintercept=0, linetype = 2) + rremove("legend")
        pdf( file.path(analysis.dir, i, "expr_integration", "PromoterDMRs", cell, paste0("scatter_expression_methylation_JMML-HCA_CB_Sampled-Epigenotype_",j,"_label.pdf")), width=4,height=4)
            print(g)
        dev.off()
        print(j)
    }
