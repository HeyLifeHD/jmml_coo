...
#subset samples for model
pheno <- pData(bsseq_all)
pheno$group <- paste0(pheno$Tissue_Source, "_", as.character(unlist(pheno$Celltype)))
pheno_sub <- pheno[pheno$Sample_Type =="tumor" | pheno$group=="cordblood_HSC",]
bsseq_sub <- bsseq_all[, rownames(pheno_sub)]
saveRDS(bsseq_sub, file.path(input.dir ,"bsseq", "bsseq_HSC_comb_snpRemoved_repMerged_sub_cbHSC_comparison.rds"))
...
#define groups
contrasts <- data.frame(groupHM=c("HM", "cordblood_HSC"), groupLM=c("LM", "cordblood_HSC"))
...
for(i in names(contrasts)){
    keepLoci[[i]] <- which(rowSums(cov_drms[[i]][,rownames(pheno[pheno$group==contrasts[1,i],])] >= 5) >= round(length(rownames(pheno[pheno$group==contrasts[1,i],]))/2) &
                     rowSums(cov_drms[[i]][,rownames(pheno[pheno$group==contrasts[2,i],])] >= 5) >= round(length(rownames(pheno[pheno$group==contrasts[2,i],]))/2))
}
...
meth_dmrs_diff <- list()
for(i in names(contrasts)){
    meth_dmrs_diff[[i]] <-rowMeans(meth_dmrs[[i]][, rownames(pheno[pheno$group==contrasts[1,i],])], na.rm=TRUE) - rowMeans(meth_dmrs[[i]][, rownames(pheno[pheno$group==contrasts[2,i],])], na.rm=TRUE)
