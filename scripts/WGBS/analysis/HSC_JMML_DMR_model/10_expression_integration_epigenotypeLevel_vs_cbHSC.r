...
contrasts <- t(expand.grid(as.character(unique(pheno[pheno$Sample_Type=="tumor", ]$Epigenotype)), "cordblood_HSC"))
...
for(i in names(dmrs_final)){
    ...
    for(j in colnames(contrasts)){
        ...
        merged[[i]][[j]]$Col <- NA
        merged[[i]][[j]]$Col <- ifelse(merged[[i]][[j]]$p_val_adj < 0.05 & merged[[i]][[j]]$avg_logFC >1 , "sign. upregulated",ifelse(merged[[i]][[j]]$p_val_adj < 0.05 & merged[[i]][[j]]$avg_logFC<(-1), "sign. downregulated", "no sign. change"))
        ...
        write.table( merged[[i]][[j]], file.path(analysis.dir,i,  "expr_integration", "allDMRs", paste0("expression_methylation_pan_JMML-HCA_CB_Sampled-Epigenotype_",j,".txt")), quote=FALSE, sep="\t", row.names=FALSE)
    }
}
...
for(i in names(merged)){
    ...
    for(j in names(merged[[i]])){
        ...
        g <- ggscatter(merged[[i]][[j]], x = "avg_logFC", y = j,xlab=paste0("Gene expression change\n[log2foldchange]","\n",name),
            ...
            dir.create(file.path(analysis.dir,i,  "expr_integration", "allDMRs"), recursive=TRUE)
        pdf(file.path(analysis.dir,i,  "expr_integration", "allDMRs", paste0("scatter_expression_methylation_pan_JMML-HCA_CB_Sampled-Epigenotype_",j,".pdf")), width=4,height=4)
            print(g)
        dev.off()
        g <- ggscatter(merged[[i]][[j]], x = "avg_logFC", y = j,xlab=paste0("Gene expression change\n[log2foldchange]","\n",name),
            ylab=paste0("Methyladion difference","\n",name),
            ...
            geom_vline(xintercept=0, linetype = 2) +
            geom_hline(yintercept=0, linetype = 2) + rremove("legend")
        pdf(file.path(analysis.dir,i,  "expr_integration", "allDMRs", paste0("scatter_expression_methylation_pan_JMML-HCA_CB_Sampled-Epigenotype_",j,"_label.pdf")), width=14,height=14)
            print(g)
        dev.off()
        print(j)
    }
...
for(i in names(merged)){
    for(j in names(merged[[i]])){
        ...
        pdf(file.path(analysis.dir,i,  "expr_integration", "PromoterDMRs", paste0("scatter_expression_methylation_pan_JMML-HCA_CB_Sampled-Epigenotype_",j,".pdf")), width=4,height=4)
            print(g)
        dev.off()
        g <- ggscatter(merged_sub, x = "avg_logFC", y = j,xlab=paste0("Gene expression change\n[log2foldchange]","\n",name),
            ylab=paste0("Methyladion difference","\n",name),
            ...
            geom_vline(xintercept=0, linetype = 2) +
            geom_hline(yintercept=0, linetype = 2) + rremove("legend")
        pdf(file.path(analysis.dir,i,  "expr_integration", "PromoterDMRs", paste0("scatter_expression_methylation_pan_JMML-HCA_CB_Sampled-Epigenotype_",j,"_label.pdf")), width=14,height=14)
            print(g)
        dev.off()
        print(j)
    }
